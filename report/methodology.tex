...
training and the remaining kept for testing. All samples were downloaded
manually.
...
\subsection{Sequence generation}\label{ssec:sequence_gen}
...
length about 14000 (denoted by the red dashed line) should contain between
enough syllables to capture sufficient information about the vocalization. The
fixed length, equating to about 300ms duration, also ensures that most sequences
...
Frequency Cepstral Coefficients (LFCC). Mel band edges were generated by first
converting a minimum and maximum frequency to the mel scale using
...
in Hz. The different band types can be visualized in
...
MATLAB provides no builtin functions for computing an MRCG, or even a
...
functions. The key parameter to tune here is the \texttt{BoxConstraint}, which
controls the trade-off between maximizing the margin between classes and
minimizing the classification error. This corresponds is the constant $C$ in
...
was selected to optimize the parameters, with $K=7$ folds. This is motivated by
other research taking the same approach with promising
results~\cite{ramashini2022robust}. MATLAB provides builtin ways to optimize
hyperparameters, which can be achieved by passing a \texttt{cvpartition} to the
\texttt{HyperparameterOptimizationOptions} argument and supplying a
...
linear SVM but this time by supplying a \texttt{OptimizeHyperparameters}
...
options that describe parameters such as the mini-batch size and the optimizing
...
In order to test against a baseline, a highly simplistic CNN architecture will
...
working with sequential data such as videos (comprised of sequential frames) and
potentially birdsong (comprised of sequential syllables). Since we don't need
...
MATLAB can receive sequential data in different input formats, such as
...
the sequence length. Here, $s=n$, $c=1$ since the images can be though of as
