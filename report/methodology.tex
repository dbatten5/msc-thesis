...
This leads to the question of how to represent these sequences as sequential
data. We propose the following two methods and discuss their implications. We
will concern ourselves only with MFCC in the following list, but the concepts
can easily be applied to other representations in the frequency domain where
each element can be thought of as a T-F unit, such as GTCC and MRCG\@.

\begin{enumerate}

  \item \textbf{Sequential vectors}. Here, the MFCC for an entire sequence is
    first extracted. Supposing we extract 13 MFCC coefficients (ignoring
    $\Delta$ and $\Delta\Delta$ values for now), then entire sequence can then
    be though of as a sequence of 13 dimensional vectors, evolving through time.
    This can be visualized in Figure~\ref{fig:mfcc_vector_sequence}.
  ...
  dashed lines}\label{fig:mfcc_vector_sequence}
    ...
    features. This may mean that the network misses out on learning from
    potentially useful patterns in the signal.

  \item \textbf{Sequential images}. Here, the MFCC representation is extracted
    for each of the 5 syllables in the sequence separately. They are then
    stacked together to form a sequence of 5 2D `images' (MFCC representations).
    This sequence can then be fed as input to the network. 
